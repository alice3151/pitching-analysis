...
st.title("⚾ ピッチング動作・運動力学解析 (2画面＆グラフ解析)")
...
if uploaded_file is not None:
    ...
    # 2画面レイアウト (左: 元動画+オーバーレイ, 右: 骨格のみデータ)
    ...
    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
        while cap.isOpened():
            ...
            if len(hip_velocities) > 1:
                fig = go.Figure()
                fig.add_trace(go.Scatter(x=time_stamps, y=hip_velocities, mode='lines', name='並進速度 (px/s)', line=dict(color='cyan', width=2)))
                fig.update_layout(
                    xaxis_title="時間 (秒)",
                    yaxis_title="速度 (px/s)",
