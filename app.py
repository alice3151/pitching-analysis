...
# MediaPipe の初期化 (solutions 経由で取得)
...
if uploaded_file is not None:
    ...
    # Poseモデルの呼び出し (with構文で安全にメモリ管理)
    with mp_pose.Pose(
        static_image_mode=False,
        model_complexity=1,  # クラウド環境に合わせた標準設定
        ...
        while cap.isOpened():
            ...
            # 6. Streamlit上にフレームを表示
            st_frame.image(image_rgb, channels="RGB", use_container_width=True)

    # リソースの解放
    cap.release()    tfile = tempfile.NamedTemporaryFile(delete=False, suffix='.mov')
    tfile.write(uploaded_file.read())
